...
contract GaugeV2Test is Test {
    ...
    function setUp() public {
        ...
        init = abi.encodeCall(LiquidBoxManager.initialize, (address(this), address(liquidBoxFactory), address(6)));
        ...
        init = abi.encodeCall(
            ...
            (address(this), address(gaugeV2), address(gaugeV2ALM), address(manager), address(liquidBoxManager))
        );
        ...
        init = abi.encodeCall(
            Minter.initialize,
            (address(this), address(voterL1), address(votingEscrow), 0xE7a23916706f8319395AC46a747983f9987cf942)
        );
        ...
        IPearl(0xE7a23916706f8319395AC46a747983f9987cf942).setDepositor(address(minter));
    ...
    function l2() public {
        ...
        init = abi.encodeCall(LiquidBoxManager.initialize, (address(this), address(liquidBoxFactoryL2), address(6)));
        ...
        init = abi.encodeCall(
            GaugeV2Factory.initialize,
            (address(this), address(gaugeV2L2), address(gaugeV2ALML2), address(manager), address(liquidBoxManagerL2))
        );
        ...
        init = abi.encodeCall(BribeFactory.initialize, (address(this), voterProxyAddressL2, ustb, addr));
        ...
        gaugeV2L2 = GaugeV2(voterL2.createGauge{value: 10 ether}(poolL2, ""));
    ...
    function test_shouldUpdatePendingReward() external {
        vote(poolL2);
        gaugeV2L2 = GaugeV2(voterL1.gauges(poolL2));
    ...
    function test_shouldBridgePendingRewardToL2() external {
        vote(poolL2);
        GaugeV2 gauge = GaugeV2(voterL1.gauges(poolL2));

        gaugeV2FactoryL1.setTrustedRemoteAddress(lzPoolChainId, address(gauge), address(gaugeV2L2));
        gaugeV2FactoryL2.setTrustedRemoteAddress(lzMainChainId, address(gaugeV2L2), address(gauge));
        console.log(address(gauge), address(nativeOFT));

        gaugeV2FactoryL1.setTrustedRemoteAddress(lzPoolChainId, address(nativeOFT), address(gaugeV2L2));
        gaugeV2FactoryL2.setTrustedRemoteAddress(lzMainChainId, address(gaugeV2L2), address(gauge));
    ...
    function mintNewPosition2(uint256 amount0ToAdd, uint256 amount1ToAdd)
        private
