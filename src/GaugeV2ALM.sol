...
contract GaugeV2ALM is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _pullRewardFromGauge() internal {
        ...
        if (liquidity != 0) {
            ...
            uint256 reward = gaugeCL.collectRewardForALM(box.baseLower(), box.baseUpper());

            if (reward > 0) {
                (uint256 rewardPerTokenGrowth, uint256 managementFeesGrowth) = _getRewardsGrowth(reward);
                unchecked {
                    managementFees += managementFeesGrowth;
                    rewardPerTokenStored = rewardPerTokenStored.add(rewardPerTokenGrowth);
                }
            }
        }
    ...
    function rebalanceGaugeLiquidity(int24 newtickLower, int24 newtickUpper, uint128, uint128) external nonReentrant {
