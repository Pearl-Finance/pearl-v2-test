...
contract GaugeV2 is IGaugeV2, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ALMGaugeChanged(address almGauge);

    event DistributionChanged(address distribution);
    ...
    function initialize(
        ...
        if ((_isMainChain && _lzMainChainId == _lzPoolChainId) || (!isMainChain && _lzMainChainId != _lzPoolChainId)) {
    ...
    /// @inheritdoc IGaugeV2
    function deposit(uint256 tokenId) external nonReentrant isStakingAllowed {
        // transfer NFT
        ...
        pullNFTFee(params.tokenId)
    {
        ...
        if (params.amount0Desired > 0) {
            ...
            IERC20Upgradeable(token0).safeIncreaseAllowance(address(nonfungiblePositionManager), params.amount0Desired);
        }

        if (params.amount1Desired > 0) {
            params.amount1Desired = _safeTransferFrom(token1, msg.sender, address(this), params.amount1Desired);
            IERC20Upgradeable(token1).safeIncreaseAllowance(address(nonfungiblePositionManager), params.amount1Desired);
        }
    ...
    function crossTo(int24 targetTick, bool zeroForOne) external nonReentrant returns (bool) {
        ...
        while (isTrue) {
            ...
            if ((zeroForOne ? targetTick >= tickNext : targetTick < tickNext)) break;
    ...
    // end to end testing
    // briding emmission using mock bridge
    // chain conditions needs to be tested
    // convertBribeToken
    ...
    function ackReward(uint64 _nonce) external nonReentrant {
        console.log(address(this), "e reach here too");
    ...
    function claimFees() external returns (uint256 claimed0, uint256 claimed1) {
        ...
        if (claimed0 > 0) {
            feeAmount.amount0 = 0;
            IERC20Upgradeable(_token0).safeIncreaseAllowance(internal_bribe, claimed0);
            IBribe(internal_bribe).notifyRewardAmount(_token0, claimed0);
        }

        if (claimed1 > 0) {
            feeAmount.amount1 = 0;
            IERC20Upgradeable(_token1).safeIncreaseAllowance(internal_bribe, claimed1);
