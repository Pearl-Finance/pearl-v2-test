...
/// @title Returns information about the token value held in a Pearl V2 NFT
library PositionValue {
    /// @notice Returns the total amounts of token0 and token1, i.e. the sum of fees and principal
    /// that a given nonfungible position manager token is worth
    /// @param positionManager The Pearl V2 NonfungiblePositionManager
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        unchecked {
            (uint256 poolFeeGrowthInside0LastX128, uint256 poolFeeGrowthInside1LastX128) = _getFeeGrowthInside(
                IPearlV2Pool(
                    IPearlV2Factory(positionManager.factory()).getPool(
                        feeParams.token0, feeParams.token1, feeParams.fee
                    )
                ),
                feeParams.tickLower,
                feeParams.tickUpper
