...
contract Bribe is IBribe, Initializable, ReentrancyGuardUpgradeable {
    ...
    event RewardAdded(uint64 indexed nonceId, address rewardToken, uint256 reward, uint256 startTimestamp);
    ...
    function initialize(
        ...
        require(_bribeFactory != address(0) && _voter != address(0) && _owner != address(0), "zero addr");
    ...
    function _addBribeForEpoch(uint64 _nonceId, address _rewardsToken, uint256 reward) internal {
        ...
        IBribe.Reward memory mRewardData = _rewardData[_rewardsToken][_startTimestamp];
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external nonReentrant {
        ...
        reward = _safeTransferFrom(_rewardsToken, msg.sender, address(this), reward);
    ...
    function skim(address _to) external onlyKeeper returns (uint256[] memory _amounts) {
    ...
    function skim(address _rewardsToken, address _to) external onlyKeeper returns (uint256 _amount) {
    ...
    function emergencyRecoverERC20AndRecoverData(bytes calldata data, bool isRecoverERC20AndUpdateData)
        external
        onlyRole
    {
        (address[] memory tokenAddress, uint256[] memory tokenAmount) = abi.decode(data, (address[], uint256[]));

        for (uint256 k = 0; k < tokenAddress.length;) {
            if (tokenAmount[k] > 0) {
                require(tokenAmount[k] <= IERC20(tokenAddress[k]).balanceOf(address(this)), "!balance");

                if (isRecoverERC20AndUpdateData) {
                    uint256 _startTimestamp = IMinter(minter).active_period() + EPOCH_DURATION;
                    uint256 _lastReward = _rewardData[tokenAddress[k]][_startTimestamp].rewardsPerEpoch;
                    _rewardData[tokenAddress[k]][_startTimestamp].rewardsPerEpoch = _lastReward - tokenAmount[k];
                    _rewardData[tokenAddress[k]][_startTimestamp].lastUpdateTime = block.timestamp;
                }
    ...
    function getReward(address[] calldata tokens) external nonReentrant isAllowed {
