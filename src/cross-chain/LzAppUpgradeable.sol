...
abstract contract LzAppUpgradeable is OwnableUpgradeable, ILayerZeroReceiver {
    ...
    uint256 public constant DEFAULT_PAYLOAD_SIZE_LIMIT = 1_00_00;

    // bytes = abi.encodePacked(remoteAddress, localAddress)
    // this function set the trusted path for the cross-chain communication
    mapping(uint16 => bytes) public trustedRemoteLookup;

    event SetTrustedRemote(uint16 indexed _remoteChainId, bytes _path);
    ...
    constructor(address _endpoint) {
        require(_endpoint != address(0), "LzApp: zero addr");
    ...
    function _checkPayloadSize(uint16, uint256 _payloadSize) internal view virtual {
        require(_payloadSize <= DEFAULT_PAYLOAD_SIZE_LIMIT, "LzApp: payload size is too large");
    }
