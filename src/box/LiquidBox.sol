...
contract LiquidBox is ILiquidBox, Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    event ClaimFees(address indexed from, address to, uint256 feesToOwner0, uint256 feesToOwner1);
    event DirectDeposit(bool isTrue);
        ...
        returns (uint256 shares, uint256 amount0, uint256 amount1)
    {
        ...
        ILiquidBoxCallback(msg.sender).boxDepositCallback(amount0Desired, amount1Desired, to);

        amount0Desired = token0.balanceOf(address(this)) - amount0Before;
        amount1Desired = token1.balanceOf(address(this)) - amount1Before;
        ...
        internal
    {
        if (liquidity > 0) {
            (uint256 amount0, uint256 amount1,) =
                pool.mint(msg.sender, address(this), tickLower, tickUpper, liquidity, "");
